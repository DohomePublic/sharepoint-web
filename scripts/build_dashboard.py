#!/usr/bin/env python3
"""
build_dashboard.py  (v3 — auto-resolve Site + List with diagnostics)
---------------------------------------------------------------------
1. Authenticate to Microsoft Graph via client-credentials flow
2. Resolve Site ID automatically — prints all sites on failure
3. Resolve List ID automatically — prints all lists on failure
4. Fetch ALL items from SharePoint KYCData1 (handles pagination)
5. Generate index.html — dark KYC Dashboard, NO credit-limit fields shown
"""
...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
# ⚠️  ถ้า workflow fail ให้ดู [DIAGNOSE] section ใน log แล้วแก้ค่าด้านล่างนี้
TENANT_ID     = os.environ.get("AZURE_TENANT_ID", "").strip()
CLIENT_ID     = os.environ.get("AZURE_CLIENT_ID", "").strip()
CLIENT_SECRET = os.environ.get("AZURE_CLIENT_SECRET", "").strip()

SITE_HOST  = "dohomegroup.sharepoint.com"
SITE_PATH  = "/sites/KYC"      # ← แก้ตรงนี้ถ้า [DIAGNOSE] บอกชื่อ site ที่ถูก
LIST_NAME  = "KYCData1"        # ← แก้ตรงนี้ถ้า [DIAGNOSE] บอกชื่อ list ที่ถูก
...
def validate_env():
    ...
    if missing:
        ...
        raise SystemExit(1)
    print(f"[OK] Secrets loaded — Tenant: {TENANT_ID[:8]}***")
...
def get_token():
    ...
    if not r.ok:
        print(f"[ERROR] Auth HTTP {r.status_code}: {r.text[:300]}")
        r.raise_for_status()
...
def resolve_site_id(token):
    """
    Tries SITE_HOST + SITE_PATH.
    On 404 → lists every accessible site so you know what to put in SITE_PATH.
    """
    print(f"[SITE] Resolving: {SITE_HOST}{SITE_PATH}")
    try:
        data = graph_get(
            token,
        ...
        print()
        print("=" * 70)
        print("[DIAGNOSE] Site not found. Listing ALL sites this App can access:")
        print("=" * 70)
        headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
        sr = requests.get(
        ...
        if sr.ok:
            sites = sr.json().get("value", [])
            print(f"  Found {len(sites)} site(s):\n")
            for s in sites[:50]:
                web = s.get("webUrl", "")
                # extract path from webUrl e.g. /sites/KYCData
                path = "/" + "/".join(web.split("/")[3:]) if "/" in web else ""
                print(f"  name        = {s.get('name')!r}")
                print(f"  webUrl      = {web}")
                print(f"  → SITE_PATH = {path!r}")
                print()
        else:
            print(f"  Could not list sites: HTTP {sr.status_code}")
            print("  Ensure 'Sites.Read.All' Application permission is granted.")
        print("=" * 70)
        raise SystemExit(
            "\n[ACTION REQUIRED]\n"
            "  Look at [DIAGNOSE] above and set SITE_PATH to the correct value.\n"
            "  Example: SITE_PATH = '/sites/KYCData'\n"
            "  Then commit + re-run the workflow."
        )
...
def resolve_list_id(token, site_id):
    """
    Tries LIST_NAME (internal name or display name).
    On 404 → lists every list in the site so you know what to put in LIST_NAME.
    """
    print(f"[LIST] Resolving: {LIST_NAME!r}")
    try:
        data = graph_get(
            token,
        ...
        lid = data["id"]
        print(f"[OK]   List ID      = {lid}")
        print(f"[OK]   displayName  = {data.get('displayName')!r}")
        return lid
    except Exception:
        print()
        print("=" * 70)
        print("[DIAGNOSE] List not found. Listing ALL lists in this site:")
        print("=" * 70)
        headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
        lr = requests.get(
        ...
        if lr.ok:
            lists = lr.json().get("value", [])
            print(f"  Found {len(lists)} list(s):\n")
            for lst in lists:
                print(f"  name        = {lst.get('name')!r}")
                print(f"  displayName = {lst.get('displayName')!r}")
                print(f"  → LIST_NAME = {lst.get('name')!r}")
                print()
        else:
            print(f"  Could not list lists: HTTP {lr.status_code}")
        print("=" * 70)
        raise SystemExit(
            "\n[ACTION REQUIRED]\n"
            "  Look at [DIAGNOSE] above and set LIST_NAME to the correct 'name' value.\n"
            "  Then commit + re-run the workflow."
        )
...
def fetch_all_items(token):
    # 1. Resolve Site (with diagnostics on failure)
    site_id = resolve_site_id(token)

    # 2. Resolve List (with diagnostics on failure)
    list_id = resolve_list_id(token, site_id)

    # Update SP_DISP with confirmed site
    global SP_DISP
    SP_DISP = f"https://{SITE_HOST}{SITE_PATH}/Lists/{LIST_NAME}/DispForm.aspx?ID="

    # 3. Paginate items using confirmed IDs
    url = (
    ...
    while url:
        ...
        data  = graph_get(token, url, f"page {page}")
        items.extend(data.get("value", []))
        url   = data.get("@odata.nextLink")
        print(f"        Cumulative: {len(items)} items", flush=True)
    return items


# ── TRANSFORM ──────────────────────────────────────────────────────────────────
def clean_biz(v):
    return v.replace(";#", "").strip(";").strip() if v else ""
...
def to_row(item):
    ...
    return {
        "id":         item_id,
        "title":      f.get("Title")               or f"Item {item.get('id','?')}",
...
def compute_stats(rows):
    ...
    last = sorted(mc.items())[-30:]
    return dict(sc), dict(tc), [{"ym": k, "cnt": v} for k, v in last]
...
CSS = (
    ...
    "font-family:'Sarabun','Noto Sans Thai',sans-serif;font-size:15px;min-height:100vh;}\n"
    ...
    ".kpi::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--kc,var(--ac));}\n"
    ".kpi .lbl{font-size:11px;color:var(--t2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;}\n"
    ...
    ".card h3{font-size:12px;color:var(--t2);margin-bottom:14px;text-transform:uppercase;letter-spacing:.05em;}\n"
    ".cw{position:relative;height:210px;}\n"
    ".frow{display:flex;gap:9px;flex-wrap:wrap;margin-bottom:12px;align-items:center;}\n"
    ".frow input,.frow select{background:var(--s2);border:1px solid var(--bd);color:var(--t);"
    "padding:8px 12px;border-radius:8px;font-size:13px;font-family:inherit;outline:none;transition:.2s;}\n"
    ".frow input{flex:1;min-width:200px;}\n"
    ".frow input:focus,.frow select:focus{border-color:var(--ac);}\n"
    ".badge-cnt{background:var(--ac);color:#0f172a;font-size:12px;font-weight:700;padding:4px 12px;border-radius:20px;}\n"
    ".btn-r{background:var(--s2);border:1px solid var(--bd);color:var(--t2);"
    "padding:8px 12px;border-radius:8px;cursor:pointer;font-size:13px;font-family:inherit;transition:.2s;}\n"
    ...
    "th{padding:11px 13px;text-align:left;font-size:11px;font-weight:600;color:var(--t2);"
    ...
    ".sb{display:inline-block;padding:2px 9px;border-radius:20px;font-size:11px;font-weight:600;}\n"
    ...
    "padding:5px 11px;border-radius:7px;cursor:pointer;font-size:12px;font-family:inherit;transition:.2s;}\n"
    ".pg button:hover,.pg button.act{background:var(--ac);color:#0f172a;border-color:var(--ac);font-weight:700;}\n"
    ".pg button:disabled{opacity:.3;cursor:not-allowed;}\n"
    ".pgi{color:var(--t2);font-size:11px;margin-left:auto;}\n"
    ".ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:1000;backdrop-filter:blur(5px);}\n"
    ...
    ".dl{font-size:10px;color:var(--t2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;}\n"
    ...
    ".btn-p{background:var(--ac);color:#0f172a;border:none;padding:9px 20px;border-radius:8px;"
    "font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;transition:.2s;}\n"
    ".btn-p:hover{background:#7dd3fc;}\n"
    ".btn-s{background:var(--s2);border:1px solid var(--bd);color:var(--t2);padding:9px 20px;"
    ...
    ".wbox{margin-top:14px;padding:11px 14px;background:rgba(250,204,21,.08);"
    "border:1px solid rgba(250,204,21,.3);border-radius:9px;font-size:12px;color:var(--t2);}\n"
...
(function initCharts(){
  ...
  new Chart(document.getElementById("cSt"),{type:"doughnut",
  ...
  new Chart(document.getElementById("cTy"),{type:"bar",
    data:{labels:["\u0e40\u0e1b\u0e34\u0e14\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19\u0e43\u0e2b\u0e21\u0e48","\u0e40\u0e1e\u0e34\u0e48\u0e21\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19"],
      datasets:[{data:[TC["\u0e04\u0e33\u0e02\u0e2d\u0e40\u0e1b\u0e34\u0e14\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19\u0e25\u0e39\u0e01\u0e04\u0e49\u0e32\u0e43\u0e2b\u0e21\u0e48"]||0,
    ...
    options:{indexAxis:"y",plugins:{legend:{display:false},tooltip:{callbacks:{
      ...
      scales:{x:{grid:{color:"#334155"},ticks:{color:"#94a3b8"}},
              y:{grid:{display:false},ticks:{color:"#94a3b8"}}},maintainAspectRatio:false}});
  new Chart(document.getElementById("cMo"),{type:"line",
    data:{labels:MC.map(function(m){return m.ym;}),
    ...
    options:{plugins:{legend:{display:false}},
      scales:{x:{grid:{color:"#334155"},ticks:{color:"#94a3b8",maxRotation:45,font:{size:9}}},
              y:{grid:{color:"#334155"},ticks:{color:"#94a3b8"}}},maintainAspectRatio:false}});
})();
function badge(s){var c=SC[s]||"#64748b";
...
function applyF(){
  ...
  var fs=document.getElementById("fSt").value,ft=document.getElementById("fTy").value;
...
function resetF(){
  ...
  document.getElementById("fSt").value="";document.getElementById("fTy").value="";applyF();}
function sortBy(col){if(sortCol===col)sortDir*=-1;else{sortCol=col;sortDir=1;}doSort(col,false);render();}
function doSort(col,keep){
  if(!keep&&sortCol!==col)sortDir=1;
  filtered.sort(function(a,b){var av=a[col]||"",bv=b[col]||"";
  ...
  var si=document.getElementById("si"+col);if(si)si.textContent=sortDir===1?"\u2191":"\u2193";}
function render(){
  var tot=filtered.length,tp=Math.max(1,Math.ceil(tot/PG));
  curPage=Math.min(curPage,tp);var s=(curPage-1)*PG,sl=filtered.slice(s,s+PG);
  document.getElementById("rCnt").textContent=tot.toLocaleString()+" \u0e23\u0e32\u0e22\u0e01\u0e32\u0e23";
  var tb=document.getElementById("tBody");
  if(!sl.length){tb.innerHTML="<tr><td colspan='7' style='text-align:center;padding:40px;color:var(--t2)'>\ud83d\udd0d \u0e44\u0e21\u0e48\u0e1e\u0e1a\u0e23\u0e32\u0e22\u0e01\u0e32\u0e23</td></tr>";}
  else{tb.innerHTML=sl.map(function(r){
  ...
  document.getElementById("pI").textContent=
    "\u0e2b\u0e19\u0e49\u0e32 "+curPage+"/"+tp+" \u00b7 "+(s+1)+"\u2013"+Math.min(s+PG,tot)+" \u0e08\u0e32\u0e01 "+tot.toLocaleString();
  var btns=[],pv=0,dl=2;
  for(var i=1;i<=tp;i++){if(i===1||i===tp||Math.abs(i-curPage)<=dl){if(pv&&i-pv>1)btns.push("...");btns.push(i);pv=i;}}
  document.getElementById("pgN").innerHTML=btns.map(function(b){
    if(b==="...")return "<span style='padding:5px 3px;color:var(--t2)'>...</span>";
    return "<button onclick='goP("+b+")' class='"+(b===curPage?"act":"")+"'>"+b+"</button>";}).join("");}
function goP(p){curPage=Math.max(1,Math.min(p,Math.ceil(filtered.length/PG)));render();
  document.querySelector(".tw").scrollIntoView({behavior:"smooth",block:"nearest"});}
function showD(id){
  var r=ALL.find(function(x){return x.id===id;});if(!r)return;curId=id;
  ...
  document.getElementById("mBo").innerHTML=
    ...
    +"<div class='wbox'>\u26a0\ufe0f \u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19 \u0e40\u0e04\u0e23\u0e14\u0e34\u0e15\u0e40\u0e17\u0e2d\u0e23\u0e4c\u0e21 \u0e41\u0e25\u0e30\u0e2b\u0e25\u0e31\u0e01\u0e1b\u0e23\u0e30\u0e01\u0e31\u0e19 \u0e44\u0e21\u0e48\u0e41\u0e2a\u0e14\u0e07\u0e43\u0e19\u0e41\u0e14\u0e0a\u0e1a\u0e2d\u0e23\u0e4c\u0e14\u0e19\u0e35\u0e49\u0e15\u0e32\u0e21\u0e19\u0e42\u0e22\u0e1a\u0e32\u0e22</div>";
...
def build_html(rows, status_cnt, type_cnt, monthly, updated_at):
    ...
    status_opts = "\n".join(f"      <option>{s}</option>" for s in status_cnt)
    kpi_html = (
        f'<div class="kpi" style="--kc:#38bdf8"><div class="lbl">\u0e23\u0e32\u0e22\u0e01\u0e32\u0e23\u0e17\u0e31\u0e49\u0e07\u0e2b\u0e21\u0e14</div>'
        f'<div class="val">{total:,}</div><div class="nt">KYC-Data</div></div>\n'
        f'<div class="kpi" style="--kc:#22c55e"><div class="lbl">Approve</div>'
        f'<div class="val">{approve:,}</div><div class="nt">{ap_pct}%</div></div>\n'
        f'<div class="kpi" style="--kc:#ef4444"><div class="lbl">Reject \u0e23\u0e27\u0e21</div>'
        f'<div class="val">{reject_total:,}</div><div class="nt">{rj_pct}%</div></div>\n'
        f'<div class="kpi" style="--kc:#facc15"><div class="lbl">\u0e23\u0e2d\u0e14\u0e33\u0e40\u0e19\u0e34\u0e19\u0e01\u0e32\u0e23</div>'
        f'<div class="val">{pending:,}</div><div class="nt">\u0e23\u0e30\u0e2b\u0e27\u0e48\u0e32\u0e07\u0e1e\u0e34\u0e08\u0e32\u0e23\u0e13\u0e32</div></div>\n'
        f'<div class="kpi" style="--kc:#a78bfa"><div class="lbl">\u0e40\u0e1b\u0e34\u0e14\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19\u0e43\u0e2b\u0e21\u0e48</div>'
        f'<div class="val">{new_cr:,}</div><div class="nt">\u0e23\u0e32\u0e22\u0e01\u0e32\u0e23</div></div>\n'
        f'<div class="kpi" style="--kc:#fb923c"><div class="lbl">\u0e40\u0e1e\u0e34\u0e48\u0e21\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19</div>'
        f'<div class="val">{add_cr:,}</div><div class="nt">\u0e23\u0e32\u0e22\u0e01\u0e32\u0e23</div></div>\n'
    )
    return (
        '<!DOCTYPE html>\n<html lang="th">\n<head>\n<meta charset="UTF-8"/>\n'
        ...
        '<style>' + CSS + '</style>\n</head>\n<body>\n'
        '<header><div class="logo">\U0001f3e0</div>'
        '<div><h1>KYC Credit Dashboard</h1>'
        '<div class="sub">DoHome Group \u00b7 KYC-Data \u00b7 \u0e44\u0e21\u0e48\u0e41\u0e2a\u0e14\u0e07\u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0e27\u0e07\u0e40\u0e07\u0e34\u0e19</div></div>'
        f'<span class="upd">\U0001f504 \u0e2d\u0e31\u0e1b\u0e40\u0e14\u0e15: {updated_at}</span></header>\n'
        '<div class="main">\n<div class="kpi-grid">\n' + kpi_html + '</div>\n'
        '<div class="charts-row">\n'
        '  <div class="card"><h3>\u0e2a\u0e16\u0e32\u0e19\u0e30\u0e04\u0e33\u0e02\u0e2d</h3><div class="cw"><canvas id="cSt"></canvas></div></div>\n'
        '  <div class="card"><h3>\u0e1b\u0e23\u0e30\u0e40\u0e20\u0e17\u0e04\u0e33\u0e02\u0e2d</h3><div class="cw"><canvas id="cTy"></canvas></div></div>\n'
        '  <div class="card"><h3>\u0e41\u0e19\u0e27\u0e42\u0e19\u0e49\u0e21\u0e23\u0e32\u0e22\u0e40\u0e14\u0e37\u0e2d\u0e19</h3><div class="cw"><canvas id="cMo"></canvas></div></div>\n'
        '</div>\n<div class="card">\n'
        ...
        '    <div class="mh"><h2 id="mTi"></h2><button class="mcl" onclick="closeM()">\u2715</button></div>\n'
        '    <div class="mb" id="mBo"></div>\n'
        '    <div class="mf">\n'
        '      <button class="btn-s" onclick="closeM()">\u0e1b\u0e34\u0e14</button>\n'
        '      <button class="btn-p" onclick="openSP()">\U0001f517 \u0e14\u0e39\u0e43\u0e19 SharePoint</button>\n'
        '    </div>\n  </div>\n</div>\n'
        '<script>\n' + data_js + JS + '\n</script>\n</body>\n</html>\n'
    )
...
def main():
    ...
    print("[FETCH] Starting SharePoint fetch...")
    raw = fetch_all_items(token)
    print(f"[OK] Total: {len(raw)} items")

    rows = [to_row(i) for i in raw]
    status_cnt, type_cnt, monthly = compute_stats(rows)

    tz_thai    = datetime.timezone(datetime.timedelta(hours=7))
    updated_at = datetime.datetime.now(tz_thai).strftime(
        "%d/%m/%Y %H:%M \u0e19. (\u0e44\u0e17\u0e22)"
    )

    print("[BUILD] Generating index.html...")
