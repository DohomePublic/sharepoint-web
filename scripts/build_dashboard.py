...
# ── AUTH ──────────────────────────────────────────────────────────────────────
def get_token():
    print("[AUTH] Authenticating to Microsoft Graph...")
...
def fetch_all_items(token):
    ...
    # NOTE: ตัด $select ออก เพื่อหลีกเลี่ยง Graph bug กับ $expand=fields
    url = (
    ...
    while url:
        ...
        if not r.ok:
            print(f"[ERROR] Fetch failed: HTTP {r.status_code}")
            print(f"        URL  : {url}")
...
def main():
    ...
    print("[FETCH] Fetching SharePoint list items...")
    raw = fetch_all_items(token)
    print(f"[OK] Total fetched: {len(raw)} items")
